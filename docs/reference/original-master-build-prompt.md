...
Unity/Unreal are explicitly out of scope: they cost build time you don't have and add a hardware/install dependency judges won't tolerate. Instead, hit AAA-adjacent _feel_ entirely in-browser using modern WebGL post-processing and procedural generation — zero external 3D assets required, so there is no art-sourcing bottleneck.
...
interface Zone {
  id: string;
  type:
    'residential_low' | 'residential_high' | 'hospital' | 'school' | 'commercial' | 'industrial';
...
interface GridState {
  ...
  weather: {
    ...
    event: 'none' | 'heatwave' | 'price_spike' | 'demand_surge';
  };
}
...
function tick(state: GridState, calib: Calibration): GridState {
  ...
  for (const zone of state.zones) {
    ...
    zone.state =
      ratio > 1 ? 'blackout' : ratio > 0.85 ? 'critical' : ratio > 0.6 ? 'warning' : 'normal';
...
interface LearnerTwin {
  ...
  correctTradeoffDecisions: number;
  blackoutsCaused: number;
  zonesSaved: number;
...
interface EIACalibrationRecord {
  regionId: string;
  avgRetailPriceCentsPerKwh: number;
...
function calibrateZone(
  zone: Zone,
  eia: EIACalibrationRecord,
  baseline: EIACalibrationRecord,
): Zone {
...
- **Right**: a scrollable `decisionLog` timeline — each entry shows the decision, the predicted delta, and the actual outcome, so a player (or a judge) can audit _why_ the score is what it is
