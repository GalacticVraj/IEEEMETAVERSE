...
| R2  | **Determinism drift** — a run stops reproducing, breaking replay verification and tests.                                                    | Medium     | High   | Single seeded `xoroshiro128+` RNG + fixed-timestep `SimClock`; `Math.random()` and wall-clock time banned in pure layers ([08](./08-coding-standards.md)). Replay verification re-runs `seed + events` and compares. `typecheck:engine` keeps non-deterministic (DOM/time) APIs out of scope.                                      |
